  ...
  bool XWindow::HandleMouseEvent(const MouseEvent & e) {
    ...
    if(GetMouseFocus() != NULL){
      AffineTransform MouseTransform;
        MouseTransform = MouseTransform.MakeTranslation(GetMouseFocus()->GetBounds().GetTopLeft().x, GetMouseFocus()->GetBounds().GetTopLeft().y);
        LOG(INFO) << GetMouseFocus()->GetParent()->GetBounds().x << "and" << GetMouseFocus()->GetBounds().x;
        //LOG(INFO) << MouseTransform.GetMatrix()[2][0] -  << "and" << MouseTransform.GetMatrix()[2][1];
        Point transformedPoint = Point(e.GetWhere().x - GetMouseFocus()->GetParent()->GetBounds().x, e.GetWhere().y - GetMouseFocus()->GetParent()->GetBounds().x) ;
        MouseEvent transformedEvent(GetMouseFocus()->GetParentWindow(), e.GetType(), transformedPoint);
      //Repaint(GetMouseFocus().GetBounds());
      LOG(INFO) << "We got mouse focus";
      /*
      if(e.GetType() == MouseEvent::mouseUp){
        LOG(INFO) << "We cleared mouse focus";
        
        return true;
      }
      */
      return GetMouseFocus()->HandleMouseEvent(transformedEvent);
    }
