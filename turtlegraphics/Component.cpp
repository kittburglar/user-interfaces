...
// TODO CS349
    for (vector<Component*>::iterator iter=this->children.begin(); iter != this->children.end(); iter++) {
      Component* c = (*iter);
      if(c->IsPointInComponent(e.GetWhere()) && c->IsVisible()){
        //LOG(INFO) << "WE IN COMPONENT" << this->GetComponentNum();
        AffineTransform MouseTransform;
        MouseTransform = MouseTransform.MakeTranslation(c->GetBounds().GetTopLeft().x, c->GetBounds().GetTopLeft().y);
        //LOG(INFO) << e.GetWhere().x - c->GetBounds().GetTopLeft().x << "and" << e.GetWhere().y - c->GetBounds().GetTopLeft().y;
        //LOG(INFO) << MouseTransform.GetMatrix()[2][0] -  << "and" << MouseTransform.GetMatrix()[2][1];
        Point transformedPoint = Point(e.GetWhere().x - c->GetBounds().GetTopLeft().x, e.GetWhere().y - c->GetBounds().GetTopLeft().y - 15) ;
        MouseEvent transformedEvent(e.GetWindow(), e.GetType(), transformedPoint);

      //if (c->HandleMouseEvent(transformedEvent)) {
      if (c->HandleMouseEvent(transformedEvent)) { 
  ...
  void Component::Paint(Graphics* g)
  {
    ...
    if(this->IsVisible())
      {
      LOG_TODO << "this component is visible";
      //Save original transform
      AffineTransform originalTransform = g->GetTransform();
      AffineTransform curTransform = originalTransform;
      Rectangle originalClip;
      //Clear Background
      //g->SetForegroundColor(100);
      ...
      //Make sure the children paint themselves
      LOG_TODO << "we about to PaintComponent";
      int colour;
      for (vector<Component*>::iterator iter=this->children.begin(); iter != this->children.end(); iter++) {
        Component* c = (*iter);
        //g->SetClipRect(this->GetBounds().GetIntersection(c->GetBounds()));
        colour = colour + 10000000;
        //for (vector<Component*>::iterator iter2=c->children.begin(); iter2 != c->children.end(); iter2++) {
        //Component* d = (*iter2);
        ...
        newTransform = newTransform.MakeTranslation(GetBounds().GetTopLeft().x, GetBounds().GetTopLeft().y);
        //newTransform.MakeScale(100,100);
        //Multiply new transform matrix with current matrix
        curTransform = originalTransform * newTransform;
        //Set clip to the intersection
    ...
    {
      LOG_TODO << "this is component is not visible";
    }
